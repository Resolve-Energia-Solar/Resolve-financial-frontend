  ...
  const sendContract = async (sale) => {
    ...
    try {
      ...
      if (missingFields.length > 0) {
        setSnackbarMessage(`Os seguintes campos obrigatórios estão faltando: ${missingFields.join(', ')}`);
      ...
      if (fetchedPayment?.results?.length > 0) {
        const paymentDetails = fetchedPayment.results.map((payment) => ({
          type: TYPE_CHOICES.find((choice) => choice[0] === payment.payment_type)?.[1] || 'Tipo não especificado',
          value: formatToBRL(payment.value),
        }));

        data.observation_payment = paymentDetails.map((payment) => `${payment.type}: ${payment.value}`).join(' | ');
      }
