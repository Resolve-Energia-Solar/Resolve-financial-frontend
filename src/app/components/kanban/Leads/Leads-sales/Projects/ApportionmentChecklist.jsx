...
function ApportionmentChecklist({ leadId = null }) {
  const [initialGenerator, setInitialGenerator] = useState({
    zip_code: '',
    address: '',
    number: '',
    complement: '',
  ...
  const confirmDelete = () => {
    if (selectedBeneficiaryIndex !== null) {
      setBeneficiaries((prev) => prev.filter((b) => b.id !== selectedBeneficiaryIndex));
  ...
  const handleBeneficiaryChange = (id, key, value) => {
    setBeneficiaries((prev) => prev.map((b) => (b.id === id ? { ...b, [key]: value } : b)));
  };

  const handleDocumentUpload = (id, file, isBeneficiary = true) => {
    if (isBeneficiary) {
      setBeneficiaries((prev) =>
  ...
  const handleRemoveDocument = (id, docIndex, isBeneficiary = true) => {
    if (isBeneficiary) {
      setBeneficiaries((prev) =>
        prev.map((beneficiary) =>
          beneficiary.id === id
              ...
              {beneficiaries.map((beneficiary) => (
                <Grid item xs={12} key={beneficiary.id}>
                  <UnitiesCardComponent
                    ...
                    handleFileUpload={(e) =>
                      handleDocumentUpload(beneficiary.id, e.target.files[0], true)
                    }
                    handleRemoveDocument={(docIndex) =>
                      handleRemoveDocument(beneficiary.id, docIndex, true)
                    }
